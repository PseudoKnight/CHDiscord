...
public class Discord {
	...
	static void Connect(String token, String guildID, CClosure callback, Environment env, Target t) {
		if(connecting) {
			CHLog.GetLogger().e(CHLog.Tags.RUNTIME, "Attempted to connect to Discord multiple times simultaneously.", t);
			return;
		}
		if(jda != null) {
			Disconnect();
		...
		connection = new Thread(() -> {
			try {
				...
				CHLog.GetLogger().e(CHLog.Tags.RUNTIME, "Could not connect to Discord. " + ex.getMessage(), t);
				Disconnect();
				return;
			} finally {
				connecting = false;
	 ...
	 * @return Guild
	 */
	static Guild GetGuild(Construct id, Target t) {
	 ...
	 * @return User
	 */
	static User GetUser(Construct id, Target t) {
		User usr;
	 ...
	 * @return Member
	 */
	static Member GetMember(Construct id, Guild guild, Target t) {
		Member mem;
	 ...
	 * @return Role
	 */
	static Role GetRole(Construct id, Guild guild, Target t) {
		Role role;
		if(id instanceof CInt) {
	 ...
	 * @return GuildMessageChannel
	 */
	static GuildMessageChannel GetMessageChannel(Construct id, Target t) {
	 ...
	 * @return GuildMessageChannel
	 */
	static GuildMessageChannel GetMessageChannel(Construct id, Guild guild, Target t) {
	...
	static MessageCreateData GetMessage(Construct m, Target t) {
	...
	static MessageEmbed GetEmbed(Construct m, Target t) {
		...
		if(embed.containsKey("color")) {
			Construct mColor = embed.get("color", t);
			MCColor color;
		...
		if(embed.containsKey("footer")) {
			Construct cFooter = embed.get("footer", t);
		...
		if(embed.containsKey("fields")) {
			...
			for(Construct entry : cFields.asList()) {
				...
				if(cField.containsKey("inline")) {
					inline = ArgumentValidation.getBoolean(cField.get("inline", t), t);
				}
	 ...
	 * @return VoiceChannel
	 */
	static VoiceChannel GetVoiceChannel(Construct id, Guild guild, Target t) {
